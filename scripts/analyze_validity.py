#!/usr/bin/env python3
"""
Level 1 — criterion validity vs human holistic labels.

Reads results/level1_master.csv. Filters cv == cv_primary (where human
labels exist). Aggregates 3 runs per (JD, prompt): median LLM score.

Compares LLM holistic score (0–3) to human_holistic_label (0–3).

For each prompt (P0, P1, P2):
...
Output: results/analysis/level1_validity.csv
"""
...
OUT_CSV    = OUT_DIR / 'level1_validity.csv'
...
def analyze_one(df, prompt_label, score_col='score'):
...
def main():
    ...
    df = df[(df['cv'] == 'cv_primary') & df['human_holistic_label'].notna()].copy()
    print(f'Filtered: {len(df)} rows for cv_primary with human labels '
          f'({df["jd_id"].nunique()} unique JDs × {df["run_id"].nunique()} runs × {df["prompt"].nunique()} prompts)')
    ...
    for p in PROMPTS:
        ...
        if sub.empty:
            continue
        rows += analyze_one(sub, prompt_label=p, score_col='score')

    out = pd.DataFrame(rows)
    out.to_csv(OUT_CSV, index=False)

    print('\n' + '=' * 100)
    print('holistic_score (0–3) — LLM vs human (per prompt)')
    print('=' * 100)
